...
class PatchedMediaResource(BaseModel):
    ...
    item_type: Optional[MediaResourceItemTypeEnum] = Field(default=None, description="Type of item this media is associated with  * `course` - Course * `unit` - Unit * `resource` - Resource * `course_unit` - Course and Unit * `course_resource` - Course and Resource * `unit_resource` - Unit and Resource * `all` - Course, Unit, and Resource")
